...
import { X, Moon, Volume2, VolumeX, Download, Trash2, HardDrive, Snowflake, Construction } from "lucide-react"
  ...
  const handleClearCache = () => {
    ...
    const animate = () => {
      ...
      if (progress < 1) {
        ...
        setIsClearing(false)
        setTimeout(() => {
          // Simulate cache building up again after clear
          const newStorage = Math.random() * 5 + 2
          setDisplayedStorage(newStorage)
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="w-full max-w-md max-h-[90vh] overflow-auto rounded-lg border border-border bg-card shadow-xl">
        ...
        <div className="p-4">
          {showDevToast && (
            <div className="mb-4 flex items-center gap-2 rounded-lg border border-amber-500/50 bg-amber-500/10 p-3 animate-in fade-in slide-in-from-top-2">
              <Construction className="h-4 w-4 text-amber-500" />
              <span className="text-sm text-amber-500">Ще в розробці</span>
            </div>
          )}
          ...
          <div className="mb-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <Switch checked={true} onCheckedChange={handleThemeToggle} />
              </div>
              ...
              <div>
                <div className="mb-2 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-foreground">Кількість снігу</span>
                  </div>
                  <span className="font-mono text-xs text-muted-foreground">
                    {snowAmount === 0 ? "Вимкнено" : snowAmount}
                  </span>
                </div>
                <Slider
                  value={[snowAmount]}
                  onValueChange={(v) => setSnowAmount(v[0])}
                  min={0}
                  max={150}
                  step={10}
                  className="w-full"
