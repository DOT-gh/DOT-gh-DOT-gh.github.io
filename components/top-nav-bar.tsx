...
export function TopNavBar() {
  ...
  const handleTeacherAccess = () => {
    if (teacherCode === "Teacher443" || teacherCode === "Teacher123") {
      router.push(`/teacher?code=${teacherCode}`)
    } else {
    ...
    <>
        ...
        <div className="flex items-center gap-1 sm:gap-3">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-48">
              ...
              <DropdownMenuSeparator />
              <DropdownMenuItem onClick={() => setShowTeacherInput(true)}>
      ...
      {showTeacherInput && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-md rounded-lg border-2 border-primary/30 bg-card shadow-xl p-6">
            <div className="flex items-center gap-2 mb-2">
              <Terminal className="h-5 w-5 text-primary" />
              <h3 className="text-lg font-semibold">Вчительська</h3>
            </div>
            <p className="text-sm text-muted-foreground mb-4">Введіть код доступу щоб перейти до панелі вчителя</p>
            <div className="space-y-3">
              <div>
                <label className="text-xs text-muted-foreground mb-1 block">Код доступу</label>
                <input
                  type="password"
                  value={teacherCode}
                  onChange={(e) => setTeacherCode(e.target.value)}
                  onKeyDown={(e) => e.key === "Enter" && handleTeacherAccess()}
                  placeholder="Teacher443"
                  className="w-full px-3 py-2 rounded-md border border-border bg-background focus:outline-none focus:ring-2 focus:ring-primary"
                  autoFocus
                />
              </div>
              <div className="flex gap-2">
                <Button onClick={handleTeacherAccess} className="flex-1">
                  Увійти
                </Button>
                <Button
                  onClick={() => {
