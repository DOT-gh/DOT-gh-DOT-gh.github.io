...
const threatIcon = (char: string, color = "#eee") =>
  L.divIcon({
    className: "",
  ...
  return (
    <>
      ...
      {threats.map((t) => {
        ...
        return (
          <Marker key={t.id} position={[t.pos.lat, t.pos.lng]} icon={threatIcon("✈")}>
            <Tooltip direction="top" opacity={1}>
              <div className="entity-label">
                {spec.name} • SPD {spec.speed_kmh} • HP {t.hp.toFixed(0)}
              </div>
            </Tooltip>
          </Marker>
        );
      ...
      {selectedCard && placementPreview && (() => {
        ...
        return (
          <Circle
            center={[placementPreview.lat, placementPreview.lng]}
            radius={spec.range_km * 1000}
            pathOptions={{
              color: "#6366f1",
              dashArray: "4 6",
